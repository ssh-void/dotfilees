...
if [ -z "$winclass" ]; then
	st -f "Iosevka:size=14:weight=Regular" -g "100x30" -n "scpad" -e tmux -f ~/.config/tmux/tmux.conf &>/dev/null 
       # st -f "Iosevka:size=14:weight=Regular" -g "120x33" -n "scpad" -e tmux -f ~/.config/tmux/tmux.conf &>/dev/null 
       # st -f "Iosevka:size=16:weight=Regular" -g "150x40" -n "scpad" -e tmux -f ~/.config/tmux/tmux.conf &>/dev/null 
else
    if [ ! -f /tmp/scpad ]; then
