...
write_msr() {
	if [ "$os" != Linux ]; then
		cpucontrol -m "$1=0" "/dev/cpuctl$2" >/dev/null 2>&1
		ret=$?
	else
		# convert to decimal
		_msrindex=$(($1))
		if [ ! -w /dev/cpu/"$2"/msr ]; then
			...
			dd if=/dev/zero of=/dev/cpu/"$2"/msr bs=8 count=1 seek="$_msrindex" oflag=seek_bytes 2>/dev/null
			ret=$?
		fi
	fi
