...
d_artist() {
	...
	mpc list artist | sort -f | dmenu -i -x 280 -y 260 -z 700 -l 8 -p artist "${dmenu_args[@]}"
}
	...
	if ((${#albums[@]} > 1)); then
		{
			printf '%s\n' "$all_name"
			printf '%s\n' "${albums[@]}" | sort -f
		} | dmenu -x 280 -y 260 -z 700 -l 8 -p album "${dmenu_args[@]}"
	else
...
d_playlist() {
	...
	track=$(mpc playlist -f "$format" | dmenu -i -x 280 -y 260 -z 700 -l 8 -p track "${dmenu_args[@]}")
	printf '%s' "${track%% *}"
}
